          ...
          messages: [
            {
              ...
              content: input.strictRetry
                ? `STRICT RETRY: Return valid JSON only. Use only supplied wardrobeItemId values. Required outfit roles must be complete unless the user explicitly asked for a single-item recommendation. If the previous attempt missed a rule, fix it using this validation feedback: ${(input.validationFeedback ?? []).join('; ') || 'none'}. Never return role="other". Never invent IDs. If no valid combination exists, return status="insufficient_wardrobe". For success, return exactly {"status":"success","candidates":[{"title":"...","overallExplanation":"...","confidenceScore":0.0,"seasonLabel":"...","formalityLabel":"...","items":[{"itemId":"...","role":"...","explanation":"..."}]}]}.`
                : 'Return JSON only. Select only supplied wardrobe item IDs. Never invent wardrobe items or IDs. Respect canonical roles top, bottom, outerwear, one_piece, shoes, accessory. Prefer coherent subtype combinations for the request context, season, weather, and formality. Return 2-3 ranked candidates when enough compatible items exist. If no valid complete combination exists, return status="insufficient_wardrobe". Never return status="success" with empty items. In json_object mode, prefer exactly {"status":"success","candidates":[{"title":"...","overallExplanation":"...","confidenceScore":0.0,"seasonLabel":"...","formalityLabel":"...","items":[{"itemId":"...","role":"...","explanation":"..."}]}]}.',
            },
            {
              role: 'user',
